...
myVideo
  .videoEdit(
    concatenate(
      Concatenate.videoSource('ski_jump').transformation(new Transformation().effect(reverse()))
    )
  )
  .videoEdit(
    concatenate(
      Concatenate.videoSource('ski_jump').transformation(new Transformation().effect(accelerate(-50)))
    )
  )
...
<template>
  <div class="App-body">
    <h3>
      Apply several transformations to the ski jump video, as shown in
      <br />
      <a
        class="App-link"
        href="https://cloudinary.com/documentation/vue_video_transformations#example_2"
        target="_blank"
      >
        Example 2
      </a>
